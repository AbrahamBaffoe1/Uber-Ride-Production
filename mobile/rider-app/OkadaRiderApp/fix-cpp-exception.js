/**
 * Fix C++ Exception Script for Okada Rider App
 * 
...
function killPortProcesses() {
  ...
  // Rider app uses port 8082 according to server.js
  const ports = [8082, 19000, 19001, 19002];
...
config.transformer = {
  ...
  // Use bytecode for better performance and stability
  // See: https://reactnative.dev/docs/build-speed#using-bytecode
...
async function main() {
  try {
    log('🔧 Starting C++ Exception Fix Script for Rider App 🔧', COLORS.GREEN);
    ...
    log('\nThis will start the Rider app with optimized settings to prevent C++ exceptions.', COLORS.CYAN);
